...
def get_second_closest_offset(cross_center, target, offset=100):
    offsets = [
        ...
        if ret:
            ...
            if closest_ball is not None and back_center is not None and angle_deg is not None:
                ...
                if not checkpoint_reached:
                    is_moving = navigate_robot(angle_deg, back_center, checkpoint,
                                               calcDist(checkpoint, arrow_center),
                                               15, is_moving)
                    if calcDist(checkpoint, arrow_center) <= 15:
                        checkpoint_reached = True
                else:
                    is_moving = navigate_robot(angle_deg, back_center, goal,
                                               calcDist(goal, arrow_center), 30,
                                               is_moving)
